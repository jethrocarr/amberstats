...
/*
	...
	* pcre		Verifes that input is valid PCRE regex
...
function security_form_input_predefined ($type, $valuename, $numchar, $errormsg)
{
	...
	switch ($type)
	{
		...
		case "pcre":
			// If the regular expression is invalid, it will return explicity false
			if (!empty($_POST[$valuename]))
			{
				if (@preg_match($_POST[$valuename], null) === false)
				{
					// there has been an error - flag the hourmins field as being incorrect input
					$_SESSION["error"]["message"][] = "Provided address is not a valid PCRE-format regular expression";
					$_SESSION["error"]["". $valuename . "-error"] = 1;
					$_SESSION["error"][$valuename] = 0;
